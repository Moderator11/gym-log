...
from app.infrastructure.models import ExerciseCategoryModel
...
class CategoryRepositoryImpl(CategoryRepository):
    ...
    def find_by_user_id(self, user_id: int) -> List[ExerciseCategory]:
        db_categories = self.db.query(ExerciseCategoryModel).filter(
            ExerciseCategoryModel.user_id == user_id
        ).order_by(ExerciseCategoryModel.name).all()
        return [self._to_domain(c) for c in db_categories]

    def find_by_id(self, category_id: int) -> Optional[ExerciseCategory]:
        db_category = self.db.query(ExerciseCategoryModel).filter(
            ExerciseCategoryModel.id == category_id
        ).first()
        if not db_category:
            return None
        return self._to_domain(db_category)

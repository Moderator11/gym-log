...
const MetricsTab = () => {
  ...
  if (isLoading) return <div className="text-center py-8 text-gray-400 dark:text-gray-500 dark:text-gray-400">불러오는 중…</div>;

  return (
    <div className="space-y-4">
      <Card>
        <h3 className="font-semibold text-gray-800 dark:text-gray-200 mb-3">새 지표 추가</h3>
      ...
      <Card>
        <h3 className="font-semibold text-gray-800 dark:text-gray-200 mb-3">내 지표 목록</h3>
        {metrics.length === 0 ? (
          <p className="text-sm text-gray-400 dark:text-gray-500 dark:text-gray-400 text-center py-6">등록된 지표가 없습니다</p>
        ) : (
            ...
            {metrics.map((m) => (
              <div key={m.id} className="flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-900 rounded-lg">
                <div>
                  ...
                  <span className="ml-2 text-sm text-gray-500 dark:text-gray-400 bg-gray-200 dark:bg-gray-700 rounded px-1.5 py-0.5">{m.unit}</span>
                </div>
                <Button
                  size="sm"
                  variant="secondary"
...
const RecordForm = ({ metrics, initial, onSubmit, onCancel }: RecordFormProps) => {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      {metrics.length === 0 ? (
        <p className="text-sm text-gray-400 dark:text-gray-500 dark:text-gray-400">지표 관리 탭에서 먼저 지표를 추가해주세요</p>
      ) : (
        <div className="space-y-3">
          <p className="text-sm font-medium text-gray-700 dark:text-gray-300">지표 값 (선택 입력)</p>
          {metrics.map((m) => (
            <div key={m.id} className="flex items-center gap-2">
              <label className="w-28 text-sm text-gray-600 dark:text-gray-300 shrink-0">
                {m.name}
                <span className="ml-1 text-xs text-gray-400 dark:text-gray-500 dark:text-gray-400">({m.unit})</span>
              </label>
              <Input
                type="number"
                step="any"
...
const RecordsTab = () => {
  ...
  return (
    <div className="space-y-4">
      ...
      {records.length === 0 ? (
        <Card>
          <p className="text-center text-gray-400 dark:text-gray-500 dark:text-gray-400 py-8">아직 건강 기록이 없습니다</p>
        </Card>
      ) : (
        <div className="space-y-3">
          {records.map((rec) => (
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6">
      <div className="flex items-center gap-3">
        ...
        <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100">건강 기록</h1>
      </div>
